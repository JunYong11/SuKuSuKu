...
//    // 👇 각 레벨별 마지막 학습 챕터 ID 리스트 만들기
//    Map<Integer, Integer> lastChapterMap = new HashMap<>();
//    for (Level level : levels) {
//        Integer lastChapterId = studyService.getLastStudiedChapterIdByLevel(loginUser, level.getLevelId());
//        lastChapterMap.put(level.getLevelId(), lastChapterId);
//    }
//    model.addAttribute("lastChapterMap", lastChapterMap);
...
//      log.info("컨(updateWordProgress)loginUser={}", loginUser);
   ...
   public String review(@RequestParam(name = "failCount", defaultValue = "1") int failCount,
         HttpSession session, Model model) {
      User loginUser = (User) session.getAttribute("loginUser");
       List<WordDto> words = studyService.reviewStudy(loginUser, failCount);  // <- 수정
      log.info("컨(reviewStudy):failCount={}", failCount);
